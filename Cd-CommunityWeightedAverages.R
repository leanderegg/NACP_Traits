...
############# Growth~Trait analysis with infilling #############
...
Nmassmod2 <- lm(log(RGR)~log.cw_Nmassp_if, biomass)
...
Nareamod2 <- lm(log(RGR)~log.cw_Nareap_if, biomass)
...
plot(log(RGR)~log.cw_LLp_if, biomass, pch=16)
abline(lm(log(RGR)~log.cw_LLp_if, biomass), lwd=2)
...
plot(log(RGR)~log.cw_LMAp_if, biomass, pch=16)
abline(lm(log(RGR)~log.cw_LMAp_if, biomass), lwd=2, lty=2)
mtext(text = paste0("p=", round(anova(LMAmod2)$`Pr(>F)`[1],2)), side=1, adj=.05, line=-1, cex=.9)
mtext("log(cwm LMA)", side=1, line=1.5, cex=1.1)
mtext("b)", side=3, adj=0)

plot(log(RGR)~log.cw_Nmassp_if, biomass, pch=16)
abline(lm(log(RGR)~log.cw_Nmassp_if, biomass), lwd=2, lty=1)
...
plot(log(RGR)~log.cw_Nareap_if, biomass, pch=16)#, yaxt="n")
#axis(2,at = log(c(.01,.02,.03,0.04,.1,0.2,0.3)), labels =c(.01,.02,.03,0.04,.1,0.2,0.3) )
abline(lm(log(RGR)~log.cw_Nareap_if, biomass), lwd=2, lty=2)
...
traitsmod <- lm(log(RGR) ~ log.cw_Nmassp_if + log.cw_LLp_if + log.cw_LMAp_if + log.cw_Nareap_if, tmpbiomass)
enviromod <- lm(log(RGR) ~ climPC1 + climPC2 + soil_N + log(LeafFrac), tmpbiomass)
summary(traitsmod) # log R2m = .1923 , only LL matters
summary(enviromod) # log R2m = 0.6424, climPC1, climPC2, and ASA matter.



summary(lm(log(RGR)~log.cw_LMAp_if, tmpbiomass[-which(tmpbiomass$PLOT_ID %in% c(43,55)),]))
summary(lm(log(RGR)~log(LeafFrac), tmpbiomass[-which(tmpbiomass$PLOT_ID %in% c(43,55)),]))

summary(lm(log(RGR)~log(ASA), tmpbiomass))
