...
# Bayesian analysis using just an Inverse Wishart, and feeding it a W matrix that has a scale:
...
model{
  ...
  sigma.raw[1:2,1:2] <- inverse(Tau.B[,])
  #for comparison with model with separate priors
  sigma.a <- sqrt(sigma.raw[1,1])
  sigma.b <- sqrt(sigma.raw[2,2])
  rho <- sigma.raw[2,1]/ (sigma.a*sigma.b) 
}

## setting up the W matrix with inits for scaling the variances.
out.lmL <- lmList(log(PLD)~I(log(temp)-log(15))|species, data=inverts)
varvec <- apply(coef(out.lmL),2,var)
W <- diag(2)*varvec
